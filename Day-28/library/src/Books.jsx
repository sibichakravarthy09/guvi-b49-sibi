...
let editdata2 = {
 status : "unavailable"
  ...
  await fetch(`https://63e0923b65b57fe60644f2ba.mockapi.io/books/${data.id}`,{
    method: "PUT",
...
if(bmd.status == "available" ){
  return <div className=" col-lg-3 mb-2 col-md-4 col-sm-6  d-flex justify-content-center data-con" key={index} >  
...
<button onClick={(() =>{ func(bmd) } )} class="btn mb-1 btn-primary" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" aria-controls="offcanvasExample">
  Details
</button>
...
<div class="offcanvas offcanvas-start text-bg-dark" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
  <div class="offcanvas-header ">
    <h2 class="offcanvas-title" id="offcanvasExampleLabel">{tempdata.name}</h2>
...
{tempdata.pages !== '' ? <h2 className="card-text">{tempdata.pages}</h2> : <h2 className="text-muted">100</h2> }
