...
import { addUser, getUser, getUserByID, resetPassword, activationMail, forgotPassword, activateAccount, generateToken, generateActivationToken, generateUserToken} from "../controller/user.js";
import jwt from "jsonwebtoken";
import nodemailer from "nodemailer";

const E_MAIL= process.env.EMAIL;
const E_PASS= process.env.PASS;
const FR_URL= process.env.FRONT_END_URL;
...
router.post("/activation", async (req, res) => {
  try {
    ...
    if (!user.isActivated) {
      ...
      const link = `${FR_URL}activate/${user._id}?activateToken=${token}`;
      const mailOptions = {
        from: E_MAIL,
        to: user.email,
        subject: "Account Activation link sent",
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    const link = `${FR_URL}authorize/?id=${user._id}&token=${token}`;
    const mailOptions = {
      from: E_MAIL,
      to: user.email,
